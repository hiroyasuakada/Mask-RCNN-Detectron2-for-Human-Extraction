...
# ##################################################################################################
    ...
    def draw_instance_predictions(self, predictions, img_num=None):
        """
        ...
        # predictions.pred_classes = predictions.pred_classes[0]
        ...
        colors = [
                [1, 1, 1] for i in range(len(classes))
            ]
        alpha = 1.0

        # if self._instance_mode == ColorMode.SEGMENTATION and self.metadata.get("thing_colors"):
        #     colors = [
        #         self._jitter([x / 255 for x in self.metadata.thing_colors[0]]) for c in classes
        #     ]
        #     # colors = None
        #     alpha = 0.8
        # else:
        ...
        return self.output, self.is_in_person
...
# ##################################################################################################
        ...
        #     if masks is not None:
        #         for segment in masks[i].polygons:
        #             self.draw_polygon(segment.reshape(-1, 2), color, alpha=0)
        #
        #     if labels is not None:
        #         # first get a box
        #         if boxes is not None:
